...
def cmmd(source, target, s_label, t_label, classnum, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    batch_size = int(source.size()[0])
    s_label = s_label.cpu()
    s_label = s_label.view(batch_size, 1)
    s_label = torch.zeros(batch_size, classnum).scatter_(1, s_label.data, 1)
    s_label = Variable(s_label).cuda()

    t_label = t_label.cpu()
    t_label = t_label.view(batch_size, 1)
    t_label = torch.zeros(batch_size, classnum).scatter_(1, t_label.data, 1)
